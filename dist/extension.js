...
const humanizeDuration = __webpack_require__(44);
...
function getWorkIssue() {
    const projectId = vscode.workspace.getConfiguration().get('gl-issues.issue.projectId');
    const iid = vscode.workspace.getConfiguration().get('gl-issues.issue.iid');
    const date = vscode.workspace.getConfiguration().get('gl-issues.issue.date');
    const time = vscode.workspace.getConfiguration().get('gl-issues.issue.time');
    return { projectId, iid, date, time };
}
// Função para salvar as credenciais
async function storeWorkIssue(projectId, iid, date, time) {
    // Aqui você pode usar o keytar para armazenar de forma segura o token (requer instalação de keytar)
...
async function fetchIssues() {
    ...
    try {
        let projectId = await getGitLabProjectId(token);
        let server = await getGitLabServer(token);
        const response = await axios_1.default.get(`https://${server}/api/v4/projects/${projectId}/issues`, {
...
async function logTimeForIssue(issueIid, timeSpent) {
    // Verifica se há credenciais salvas
    let { username, token } = getStoredCredentials();
    // Se não tiver credenciais, solicita ao usuário
    if (!username || !token) {
    ...
    try {
        let projectId = await getGitLabProjectId(token);
        let server = await getGitLabServer(token);
        const response = await axios_1.default.post(`https://${server}/api/v4/projects/${projectId}/issues/${issueIid}/add_spent_time`, { duration: timeSpent }, { headers: { 'Authorization': `Bearer ${token}` } });
        return response.data;
    }
    catch (error) {
...
async function logTimeOnIssue() {
    ...
    try {
        ...
        return logTimeForIssue(selectedIssue.iid, timeSpent);
    }
    catch (error) {
        vscode.window.showErrorMessage('Erro ao buscar issues: ' + error.message);
    }
...
async function stopWorkIssue() {
    ...
    try {
        ...
        let server = await getGitLabServer(token);
        let timeSpent = humanizeDuration(time);
        const response = await axios_1.default.post(`https://${server}/api/v4/projects/${projectId}/issues/${iid}/add_spent_time`, { duration: timeSpent }, { headers: { 'Authorization': `Bearer ${token}` } });
        storeWorkIssue("", "", 0, 0);
...
async function startWorkIssue() {
    ...
    try {
        ...
        let pId = await getGitLabProjectId(token);
        let server = await getGitLabServer(token);
        const issues = await fetchIssues();
        if (issues.length === 0) {
            vscode.window.showErrorMessage('Nenhuma issue disponível.');
...
function extractGitRemoteServer(gitUrl) {
...
function extractProjectNameFromGitUrl(gitUrl) {
    // Exemplo de URL GitLab: git@gitlab.com:user/repo.git ou https://gitlab.com/user/repo.git
    const server = extractGitRemoteServer(gitUrl);
...
async function fetchGitLabProjectId(projectPath, token) {
    try {
        let server = await getGitLabServer(token);
        const response = await axios_1.default.get(`https://${server}/api/v4/projects/${encodeURIComponent(projectPath)}`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
...
async function getGitLabProjectId(token) {
...
async function getGitLabServer(token) {
    const gitUrl = await getGitRemoteUrl();
    if (!gitUrl) {
        vscode.window.showErrorMessage('Nenhum repositório Git encontrado no projeto aberto.');
        return undefined;
